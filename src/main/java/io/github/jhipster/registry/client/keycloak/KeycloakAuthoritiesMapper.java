...
/**
 * Implementation of {@link GrantedAuthoritiesMapper}
 * and {@link JwtAuthenticationConverter} for use with Keycloak.
 *
 ...
 *
 * The {@link JwtAuthenticationConverter} part is needed if used within a OAuth2 resource server.
 * In this case {@link SimpleGrantedAuthority}s are extracted from the claims of the JWT token
 * (i.e. currently no UserInfo support).
 *
 * @author Thomas Pasch
 */
public class KeycloakAuthoritiesMapper extends JwtAuthenticationConverter implements GrantedAuthoritiesMapper
{
     ...
     * Extract SimpleGrantedAuthority from Jwt
     */
    @Override
    protected Collection<GrantedAuthority> extractAuthorities(Jwt jwt) {
        Set<String> groups = new HashSet<>();
        Set<GrantedAuthority> extractedAuthorities = new HashSet<>();

        // TODO tp: Support UserInfo as well
        OidcUserInfo userInfo = null;
        if (preferUserInfo == false || userInfo == null) {
            // mappedAuthorities.add(new SimpleGrantedAuthority(AuthoritiesConstants.USER));
            addToGroups(groups, jwt.getClaims());
        } else {
            addToGroups(groups, userInfo.getClaims());
        }

        convertGroupsToGrantedAuthorities(extractedAuthorities, groups);
        return extractedAuthorities;
    }

    /**
     * Implementation of GrantedAuthoritiesMapper
     * Map OidcUserAuthority to SimpleGrantedAuthority
     */
    @Override
