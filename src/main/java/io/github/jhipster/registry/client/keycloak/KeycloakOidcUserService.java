 ...
 * <p>
 * TODO tp:
 * Currently also uses the {@link JwtDecoder}s of {@link AdditionalIssuersForResourceServer}.
 * But this seems to be _wrong_:
 *
 * There is one {@link org.springframework.security.oauth2.client.oidc.authentication.OidcAuthorizationCodeAuthenticationProvider}
 * for each (configured) Provider (e.g. kc realm) and it contains its related OAuth2UserService/OidcUserService.
 *
 * Hence, the real 'clean' solution would be to use the JwtDecoder from the (injected?) OAuth2UserService/OidcUserService.
 * At present this does not work as all OidcAuthorizationCodeAuthenticationProviders use one KeycloakOidcUserService.
 *
 * @author Thomas Pasch
 */
public class KeycloakOidcUserService extends OidcUserService implements ApplicationContextAware {
    ...
    @Autowired
    private AdditionalIssuersForResourceServer additionalIssuersForResourceServer;
    ...
    void init() {
        ...
        if (additionalIssuersForResourceServer == null) {
            additionalIssuersForResourceServer = applicationContext.getBean(AdditionalIssuersForResourceServer.class);
        }
    ...
    private Jwt parseJwt(String accessTokenValue) {
        ...
        try {
            ...
            result = jwtDecoder.decode(accessTokenValue);
        } catch (JwtException e) {
            exception = e;
            if (additionalIssuersForResourceServer != null) {
                for (JwtDecoder decoder : additionalIssuersForResourceServer.additionalJwtDecoders()) {
                    try {
                        result = decoder.decode(accessTokenValue);
        ...
        if (result == null) {
            throw new OAuth2AuthenticationException(INVALID_REQUEST, exception);
        }
        return result;
    }
}
